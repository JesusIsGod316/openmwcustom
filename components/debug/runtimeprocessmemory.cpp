#include "runtimeprocessmemory.hpp"

#include "runtimediagnostics.hpp"

// Platform headers stay private: windows.h defines near/far and other macros
// that must not leak through a shared header into engine or Qt consumers.
#ifdef _WIN32
#ifndef NOMINMAX
#define NOMINMAX
#endif
...
namespace Debug::RuntimeDiagnostics
{
    void sampleProcessMemory() noexcept
    {
        ...
        recordEvent("os_memory", "windows", {}, {{"process_valid", processOk}, {"physical_valid", memoryOk},
...
#else
        recordEvent("coverage", "os_memory", "Windows process counters unavailable on this platform", {{"available", 0}});
#endif
    }
}
