...
new = old + " / openmw-custom-v3.19-cpu-p0"
if text.count(old) != 1:
    raise RuntimeError(f"V3.19 identity anchor mismatch: {text.count(old)}")
text = text.replace(old, new, 1)
if text.count("openmw-custom-v3.19-cpu-p0") != 1:
    raise RuntimeError("V3.19 identity marker insertion failed")
engine.write_text(text, encoding="utf-8", newline="\n")
print("V3.19 executable identity extended")
