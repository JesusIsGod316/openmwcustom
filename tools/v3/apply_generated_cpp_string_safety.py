...
def replace_label_line(rel: str, label: str, replacement: str) -> None:
    path = ROOT / rel
    lines = path.read_text(encoding="utf-8").splitlines()
    matches = [i for i, line in enumerate(lines) if label in line]
    if len(matches) != 1:
        raise RuntimeError(f"{rel}: expected one generated line containing {label!r}, found {len(matches)}")
    lines[matches[0]] = replacement
    path.write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
    print(f"generated-string safety patched {rel}: {label}")
