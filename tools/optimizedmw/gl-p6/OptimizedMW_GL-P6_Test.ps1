...
$ProfilesRoot = Join-Path $UserOpenMW 'OptimizedMW-GL-P6-Profiles'
...
Write-Host ''
Write-Host 'OptimizedMW P6 stutter-residency benchmark launcher' -ForegroundColor Cyan
Write-Host '  1 = P4R-STAGE2                  - current Stage-2 control'
...
$ProfileDir=Join-Path $ProfilesRoot ("OptimizedMW_GL-P6_{0}_{1}" -f $Experiment,$stamp)
...
try{
    ...
    @(
        ...
        "expected_lineage=optimizedmw/gl-p6-stutter-residency",
    ...
    Write-Host ''
    Write-Host "Starting OptimizedMW P6 test: $Experiment" -ForegroundColor Green
...
Write-Host ''
Write-Host 'P6 profile complete. Your normal settings have been restored.' -ForegroundColor Green
